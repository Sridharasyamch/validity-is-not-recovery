...
candidate set for \textbf{91.5\%} of them (median 72 candidates). The one-edit
candidate space is therefore operationally useful for most real generator
failures, and is not an artefact of how we built the benchmark, although
intended recovery cannot be established here without ground truth. For the remaining 8.5\% no valid molecule lies within one
...
these are real failures for which no ground truth exists; only that a one-edit
...
enumerator. Section~\ref{sec:exp7}
