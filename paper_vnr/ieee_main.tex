...
toolkit will parse, and a growing body of work aims to \emph{correct} them. That
literature is evaluated almost entirely by \emph{validity}: the fraction of
repaired strings that parse. We show that validity is a weak proxy for what
actually matters, namely whether the \emph{intended} molecule is
recovered, and that validity-only evaluation can substantially mis-rank
...
intended molecule for only 1.2\% of inputs: it emits roughly 74 valid molecules
for every correct one. An unconstrained large-language-model repair loop shows
...
un-kekulizable. Depending on architecture and sampling temperature, between a
few percent and a large minority of sampled strings fail to
parse~\citep{brown2019guacamol,polykovskiy2020moses}.
...
broken string and repair it. Correction is attractive because invalid outputs
are not noise. They are concentrated in the regions of chemical space where the
model is least certain, and discarding them biases the sampled distribution;
indeed, invalid SMILES appear to carry useful training
signal~\citep{skinnider2024invalid}.

Both prevention and correction are, however, almost always scored the same way:
by \emph{validity}, the fraction of outputs a toolkit will parse. Validity is
easy to compute, needs no ground truth, and is the headline number in most of
this literature. That it is not the whole story has been argued
before~\citep{amrec,skinnider2024invalid}. What has not been established is how
...
\item A reformulation of recovery as constrained selection, and a six-stage
      neuro-symbolic system, \ours{}, that attains 100.0\% validity and 80.7\%
      exact recovery (Sections~\ref{sec:method}--\ref{sec:exp4}).
...
invalid strings back to valid ones~\citep{schoenmaker2023uncorrupt}. A more recent
approach converts an invalid SMILES into SELFIES via grammar rules, obtaining
essentially complete validity~\citep{smiself2025}. Both are evaluated primarily
by validity, and~\citet{smiself2025} additionally report that language-model
correction is of limited benefit, a conclusion our results qualify, since it
is drawn under validity-based scoring.
...
component merely check, we let the symbolic component define a sound and
near-complete hypothesis space and use the neural component only to rank within
it.
...
development and test), the generator-distribution benchmark, the enumerated
candidate sets, all per-item system outputs, the trained character-level
generator, and the code reproducing every table and figure are prepared for
public release at \textcolor{red}{[REPOSITORY URL TO BE INSERTED]}. Source
molecules are from the public GuacaMol/ChEMBL corpus. The code is released
