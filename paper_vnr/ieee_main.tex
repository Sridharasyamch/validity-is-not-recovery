...
mix calibrated to real generative-model failures. On
...
one. An unconstrained large-language-model repair loop shows the opposite profile
(47.7\% validity, 13.8\% recovery), so the two are ordered one way by validity
and the other by recovery.
We then show that invalid-SMILES recovery is better formulated as constrained
candidate \emph{selection} than as sequence generation. A symbolic
...
supports a pruning rule selected to be lossless on the development split; and a
character-level chemical language model ranks what remains.
...
on 245{,}000 repair pairs, which our method needs none of.
The \ours{} ranker is trained only on a disjoint molecule split, so its headline
result does not depend on web-pretrained recall of test molecules. It applies to
real generative-model output: 91.5\% of 4{,}009 genuinely invalid strings from
a character-level generator have a non-empty one-edit candidate set, and on
corruptions of that generator's own molecules recovery is 78.9\%. On an external benchmark built with another group's corruption code and corpus,
recovery among reachable items is 87.5\%, and output is always valid where it
answers.
Coverage collapses from 99.7\% to 0.5\% once two errors coincide, bounding the
scope. Ranking confidence supports abstention: 46\% of inputs answer at 96.4\%
precision. We release the benchmark, code, and per-item outputs.
\end{abstract}
...
To ask it we build a leakage-controlled benchmark of corrupted SMILES with known
ground truth, drawn from a held-out molecule pool and calibrated to the error mix
a real generative model actually produces (Section~\ref{sec:bench}). On that
benchmark we measure how far validity and recovery come apart, and find the gap
wide enough to reverse the ordering of published systems: that validity is
insufficient has been argued qualitatively~\citep{amrec}, and we quantify the
consequence (Section~\ref{sec:exp1}).

The method follows from the measurement. We take the bounded-edit
constrained-selection formulation from syntax repair~\citep{considine2025syntax}
and ask what it takes to make it work for molecules, instantiating it with a
chemistry toolkit's diagnosis as the pruning constraint and a chemical language
model as the prior. The resulting system, \ours{}, attains 100.0\% validity and
80.7\% exact recovery (Sections~\ref{sec:method}--\ref{sec:exp4}), and the two
halves can be separated: enumeration makes recovery possible, covering the
ground truth for 99.79\% of items, while on that fixed pruned candidate set
selection raises exact recovery from 13.6\% under uniform choice to 80.7\% with
the domain language model. For a point of comparison outside its own family, we
train a supervised corrector in-distribution on 245k repair pairs; \ours{} beats
it without using any repair supervision at all (Section~\ref{sec:exp5}).
...
Closest to our setting are systems that repair broken SMILES directly. Learned
correction of SMILES syntax dates at least to SCROP, which couples a
retrosynthesis transformer with a neural syntax corrector~\citep{zheng2020scrop}.
Sequence
...
intended molecule. ChemFixer~\citep{park2026chemfixer} fine-tunes a
masked-pretrained transformer on real rather than synthetic failures: the invalid strings a transformer
VAE emits while learning to reconstruct molecules, each paired with its
reconstruction target. It reports validity and correction rate with
distribution-level measures (FCD, SNN), and notes that corrected molecules need
not match the original, examining mismatches through their Bemis--Murcko
scaffolds. None of these works is
validity-only. What none of them does, and what this
...
Measuring recovery requires ground truth, which failures from a freely sampling
generative model do not come with. We therefore construct corruptions of known molecules, and
...
operators, so that the synthetic corruptions follow the mix of error types a real
model makes. The mix actually realised in the benchmark is close but not identical
...
\item \textbf{Diagnose.} RDKit is applied in two stages (parse, then sanitise),
...
\item \textbf{Enumerate.} All strings within one token edit of $\tilde{s}$ are
...
\item \textbf{Verify.} Each candidate is parsed and sanitised; survivors are
...
\item \textbf{Prune.} The diagnosis constrains which edits could plausibly have
...
\item \textbf{Rank.} Surviving candidates are scored by the log-likelihood of a
...
\item \textbf{Recover or abstain.} The top-ranked candidate is returned. The margin
...
\begin{table*}[t]
\centering\small
\caption{Recovery on the held-out test split ($n=4{,}200$). Validity and exact
recovery are overall rates; Tanimoto similarity and scaffold match are
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_validity_recovery_plane.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{fig_candidate_space.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_ranking_ladder.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{fig_per_category.pdf}
\caption{Exact recovery by error category.}
\label{fig:cat}
\end{figure*}

\input{table_examples_ieee}

\paragraph{Free generation is poorly suited to this repair setting}
...
\paragraph{One-edit valid candidates exist for most real failures}
Enumerating single-token repairs for the real failures yields a non-empty
candidate set for \textbf{91.5\%} of them (median 72 candidates). A non-empty
set is necessary for recovery but not sufficient, since two-error strings often
admit a valid one-edit repair that is not the intended molecule
(Section~\ref{sec:exp7}); 91.5\% is therefore an upper bound on the share of these
failures the method can recover, not an estimate of it. For the remaining 8.5\% no valid molecule lies within one
token edit, so they fall outside the current hypothesis space; we return them
unchanged rather than guessing, which is the correct behaviour for a system that
is asked to preserve identity. We cannot say why those sets are empty, since
these are sampled failures for which no ground truth exists; only that a one-edit
enumeration does not reach a valid molecule.

\paragraph{Identity preservation without ground truth}
Exact recovery cannot be measured on these failures: a generator sampling
freely records no intended molecule, and that is precisely why the controlled
benchmark exists. What can be measured is how much of the string a repair destroys. A
...
Recovery carries over to the molecules a real model produces, losing under two
points. The errors are still our operators, so this varies the molecules rather
than the error process. Note also that the ranker here is the very model that generated the
...
almost no use at $k=2$, which fixes the method's scope boundary. The $k=2$ row
also qualifies Section~\ref{sec:exp6}: 276 of those 600 strings still have a
non-empty candidate set, yet only 3 of those sets contain the intended molecule,
so a non-empty set does not show that a real failure is a single edit. Our data
cannot settle how many real failures fall inside the boundary;
\citet{park2026chemfixer} describe one to five erroneous tokens per sequence as
typical in their MOSES setting, which suggests the multi-edit case is not marginal. On the $k=1$ benchmark, whose molecules
...
against. For the 15.5\% it declines, no valid molecule lies within one token
edit of the input. That is not the same as saying each carries several errors:
their operators permute tokens and graft whole fragments, either of which can
leave the one-edit space in a single application. Declining them is the correct
behaviour for a system whose purpose is to preserve identity.
...
The six prespecified pairwise comparisons below are significant under exact
paired McNemar tests with Bonferroni correction ($m=6$):
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_risk_coverage.pdf}
...
Symbolic enumeration makes recovery \emph{possible}: it delivers a sound
candidate set containing the answer 99.79\% of the time. The chemical prior
makes recovery \emph{accurate}, raising exact recovery on the same pruned
candidate set from 13.6\% with uniform selection to 80.7\%. Neither half
suffices alone. Enumeration without a prior already beats the published
corrector we evaluate by an order of magnitude (13.6\% against 1.2\%) but falls
well short of a supervised model trained for the task (59.4\%); a prior without
enumeration, the free LLM loop, sacrifices validity and drifts off the intended
molecule.

\paragraph{Limitations}
Our benchmark applies single-token corruptions. This is a deliberately narrow
hypothesis space; Section~\ref{sec:exp6} shows that 91.5\% of real generator
failures have a non-empty one-edit valid candidate set, while the remaining
8.5\% have no valid repair within that space and are outside the current
enumerator. A non-empty set bounds recoverability without establishing it,
and free sampling records no target to measure it against; an autoencoder's
reconstruction failures do~\citep{park2026chemfixer}, and would close this gap. Section~\ref{sec:exp7}
...
Section~\ref{sec:exp8} shows no observed drop in conditional recovery on an
external molecule corpus it never saw, but the cost of a deliberately
adversarial mismatch remains uncharacterised.
